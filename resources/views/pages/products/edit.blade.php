        ...
        <section class="section">
            ...
            <div class="section-body">
                @include('components.help_panel', [
                    'id' => 'help-products-edit',
                    'title' => 'Panduan singkat • Produk & Resep',
                    'items' => [
                        'Step 1: Ubah detail produk bila diperlukan.',
                        'Step 2 (opsional): Centang "This product has a recipe" untuk menyalakan/memperbarui resep. Isi <em>Yield Quantity</em>, <em>Unit</em>, dan bahan-bahan.',
                ...
                <div class="card">
                    <form id="product-wizard-form" action="{{ route('product.wizard.update', $product) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="card-body">
                            <div id="step-1">
                                ...
                                <div class="form-group">
                                    <label>Stock</label>
                                    <input type="number" step="0.01" class="form-control @error('stock') is-invalid @enderror" name="stock" value="{{ old('stock', $product->stock) }}">
                            ...
                            <div id="step-2" style="display:none">
                                ...
                                @php($hasRecipe = isset($recipe) && $recipe->id && $recipe->items && $recipe->items->count() > 0)
                                <div class="form-check mb-3">
                                    <input class="form-check-input" type="checkbox" id="recipe_enabled" name="recipe_enabled" value="1" {{ old('recipe_enabled', $hasRecipe) ? 'checked' : '' }}>
                                ...
                                <div id="recipe-fields" style="display: none;">
                                    <div class="form-row">
                                        <div class="form-group col-md-6">
                                            ...
                                            <input type="number" step="0.0001" class="form-control @error('yield_qty') is-invalid @enderror" name="yield_qty" value="{{ old('yield_qty', $recipe->yield_qty ?? '') }}">
                                            @error('yield_qty')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                        </div>
                                        <div class="form-group col-md-6">
                                            <label>Unit (e.g. pcs, cup)</label>
                                            <input type="text" class="form-control @error('unit') is-invalid @enderror" name="unit" value="{{ old('unit', $recipe->unit ?? '') }}">
...
(function() {
    ...
    function recipeEnabled() { return $('#recipe_enabled').is(':checked'); }
    function toggleRecipeFields() { $('#recipe-fields').toggle(recipeEnabled()); }
    ...
    $(function() {
        ...
        let seed = oldItems.length ? oldItems : (recipeEnabled() ? (existingItems.length ? existingItems : [{}]) : []);
        seed.forEach(i => addItemRow(i));
